...
## Prepare chromatin structure profile data
occ <- as.data.frame(t(cbind(list.files("mappingA_new",pattern="A6.n.*bw", full=TRUE), list.files("mappingD",pattern="Dc.*bw", full=TRUE), list.files("mappingF",pattern="Fc.*bw", full=TRUE), list.files("mappingMnew",pattern="Mc.*bw", full=TRUE))))
names(occ) = c("occD","occH","occL")
...
## Prep and run deeptools script "aggreNvisual.deeptools.sh"
