...
options(scipen=999)
setwd("/work/LAS/jfw-lab/hugj2006/cottonLeaf/RNAseq/RanalysisIndivGNTr")
...
dir <- "/work/LAS/jfw-lab/hugj2006/cottonLeaf/RNAseq/mappingIndivR/"
# individal A2 ref
...
# A   0.04173372   0.04792720   0.05268946
# D   0.03767067   0.04199956   0.04741882
...
# A   0.02087224   0.02051357   0.02956586
# D   0.02302950   0.02169117   0.02941873

# relative TPM of panTEs
# values only slightly lower than above, showing panTE account for the majority of TE expression
colSums(A2f$tpmTE[,4:6])/(colSums(A2$tpmTE[,4:6]) + colSums(A2$tpm[,1:3]))
# SD5-A2-S1  SD5-A2-S4  SD5-A2-S5
#0.02667614 0.02341341 0.02295299
colSums(D5f$tpmTE[,4:5])/(colSums(D5$tpmTE[,4:5]) + colSums(D5$tpm[,1:2]))
# SD5-D5-S1  SD5-D5-S3
#0.06330032 0.04172624
...
#  SD5-Maxxa-S1 SD5-Maxxa-S2 SD5-Maxxa-S3
#A   0.04045770   0.04651083   0.05120160
#D   0.03651117   0.04063738   0.04585518
...
#  SD5-A2xD5-S1 SD5-A2xD5-S2 SD5-A2xD5-S3
# A   0.01976156   0.01942461   0.02844581
# D   0.02166890   0.02064558   0.02814819
...
#  quick check up
lapply(AD1f,dim); #45778 20035
lapply(A2f,dim);  #22889 20035
...
lapply(D5f,head)
lapply(F1f,head)
save(A2,D5,F1,AD1,A2f,D5f,F1f,AD1f,file="expression.IndivGNTrRNA.rdata")


################################################
## Compare Individual ref GNT GENE expression ##
################################################
load("expression.IndivGNTrRNA.rdata")->l;l
# for convinience
# check A vs D
rbind(
c("Diploids colSums A2/D5",colSums(A2f$tpm[,1:3])/c(colSums(D5f$tpm[,c(1,3)]),NA)),
c("AD1 colSums A/D",colSums(AD1f$tpm[grep("A",rownames(AD1f$tpm)),1:3])/colSums(AD1f$tpm[grep("D",rownames(AD1f$tpm)),1:3])),
c("F1 colSums A/D",colSums(F1f$tpm[grep("A",rownames(F1f$tpm)),1:3])/colSums(F1f$tpm[grep("D",rownames(F1f$tpm)),1:3]))
)
# SD5-A2-S1           SD5-A2-S4           SD5-A2-S5
# [1,] "Diploids colSums A2/D5" "1.09223897400044"  "10.5448875671866"  NA
# [2,] "AD1 colSums A/D"        "0.96380749862041"  "0.981157894868057" "0.999755193389643"
# [3,] "F1 colSums A/D"         "0.941061535885921" "0.953688196028842" "0.96275665912134"
# Different pattern, Based on AD1 ref: F1 A/D >1 biasedly assign reads to A genome, but AD1 < or = 1; this is different from D5 ref results
...
#        DNA/DTA      DNA/DTC      DNA/DTH      DNA/DTM      DNA/DTT DNA/Helitron    LTR/Copia    LTR/Gypsy  LTR/unknown
# 175          233           54          434           23           10          953         3143         1407
...
for(nkm in 4:10){
    ...
    par(mfrow=c(3,4))
    for(i in 1:nkm){
        pie(table(idx$class[c[[i]]]), col=pcol,clockwise=TRUE,main=i)
    }
}
dev.off()
...
# A2 vs D5
count = cbind(A2f$count,D5f$count)
...
dds <- DESeqDataSetFromMatrix( countData = round(count,0), colData =info, design = ~ genome)
colSums(count)
libTotal = c(colSums(F1$count),colSums(F1$count))
libSize = libTotal/mean(libTotal)
sizeFactors(dds) = libSize
res = results(DESeq(dds),contrast=c("genome","A","D"))
print( summary(res,alpha=.05) ) # Higher: A 1626 D 2001
write.table(res, file="DE.F1.AtvsF1.Dt.txt",  sep="\t")
# Get allelic expression divergence in F1, test for B=0
B=res

# AD1: At vs Dt
count = cbind(AD1f$count[grep("A$",rownames(AD1f$count)),], AD1f$count[grep("D$",rownames(AD1f$count)),])
rownames(count) =gsub(".A$","",rownames(count))
names(count)=paste(names(count),c("A","A","A","D","D","D"),sep=".")
info = data.frame(sample=names(count), genome = c("A","A","A","D","D","D"))
dds <- DESeqDataSetFromMatrix( countData = round(count,0), colData =info, design = ~ genome)
...
sizeFactors(dds) = libSize
res = results(DESeq(dds),contrast=c("genome","A","D"))
print( summary(res,alpha=.05) ) # Higher: A 2122 D2400
write.table(res, file="DE.AD1.AtvsAD1.Dt.txt",  sep="\t")
# homoeolog expression divergence in polyploid, test for Bp=0
Bp=res

# F1 vs AD1
count = cbind(F1f$count,AD1f$count)
info = data.frame(sample=names(count), genome = gsub("SD5-|-S.","",names(count)))
dds <- DESeqDataSetFromMatrix( countData = round(count,0), colData =info, design = ~ genome)
libTotal = c(colSums(F1$count),colSums(AD1$count))
libSize = libTotal/mean(libTotal)
sizeFactors(dds) = libSize
res = results(DESeq(dds),contrast=c("genome","Maxxa","A2xD5"))
print( summary(res,alpha=.05) ) # higher: AD1 4209, F1 4452
...
F1.t = getTotal(F1f$count)
AD1.t = getTotal(AD1f$count)
libTotal =c(colSums(A2$count[,1:3]),colSums(D5$count[,1:2]))
libSize = libTotal/mean(libTotal)
mid = as.data.frame(cbind(A2f$count[,1]/libSize[1]+D5f$count[,1]/libSize[4], A2f$count[,1]/libSize[1]+D5f$count[,2]/libSize[5], A2f$count[,2]/libSize[2]+D5f$count[,1]/libSize[4], A2f$count[,2]/libSize[2]+D5f$count[,2]/libSize[5], A2f$count[,3]/libSize[3]+D5f$count[,1]/libSize[4], A2f$count[,3]/libSize[3]+D5f$count[,2]/libSize[5]))/2
names(mid)=paste0("SD5-mid-",c("S11","S12","S21","S22","S31","S32"))
total = cbind(A2f$count,D5f$count,F1.t,AD1.t,mid)
colSums(total)
c(colSums(A2$count),colSums(D5$count),colSums(F1$count),colSums(AD1$count), rep(mean(libTotal),6) )
...
libTotal=c(colSums(A2$count),colSums(D5$count),colSums(F1$count),colSums(AD1$count), rep(mean(libTotal),6) )
libSize = libTotal/mean(libTotal)
...
print( summary(res,alpha=.05) ) # higher: F1 3880 Mid 3950
...
print( summary(res,alpha=.05) ) # higher: AD1 4115, Mid 3564
...
print( summary(res,alpha=.05) ) # higher: F1 3012, A2 2075
...
print( summary(res,alpha=.05) ) # higher AD1 3089 A2 2047
...
print( summary(res,alpha=.05) ) # higher F1 1814, D5 1227
...
print( summary(res,alpha=.05) ) # higher AD1 1353, D5 1112
...
print( summary(res,alpha=.05) ) # higher Mid 3515, A2 1476
...
print( summary(res,alpha=.05) ) # higher Mid 2067, D5 1670
...
# "Diploid divergence, A2vsD5"       "A"       "2406" "2874"
# "Homoeolog bias, F1"               "B"       "1626" "2001"
# "Homoeolog bias, AD1"              "Bp"      "2122" "2400"
# "Hybridization effect direction"   "Hr"      "366"  "443"
# "Allopolyploidy effect direction"  "Pr"      "1329" "1320"
# "Genome doubling effect direction" "Wr"      "1206" "1131"
