...
TSS.A = TSS.all[grep("A",seqnames(TSS.all))]
TSS.D = TSS.all[grep("D",seqnames(TSS.all))]
# OG 22889 groups
...
og<-data.frame(D5=,AD1=c(ogQ$V3,ogQ$V4))
TSS.Aog = TSS.all[ogQ$V3]
TSS.Dog = TSS.all[ogQ$V4]
## DE based AD1
load("RNAseq/RanalysisAD1GNT/diffExprPattern.rdata")->l;l
Abias.A= ogQ$V3[match( rownames(Bp[Bp$padj<0.05&Bp$log2FoldChange>0&!is.na(Bp$padj),]), ogQ$V5)]
Abias.D= ogQ$V4[match( rownames(Bp[Bp$padj<0.05&Bp$log2FoldChange>0&!is.na(Bp$padj),]), ogQ$V5)]
Dbias.A= ogQ$V3[match( rownames(Bp[Bp$padj<0.05&Bp$log2FoldChange<0&!is.na(Bp$padj),]), ogQ$V5)]
Dbias.D= ogQ$V4[match( rownames(Bp[Bp$padj<0.05&Bp$log2FoldChange<0&!is.na(Bp$padj),]), ogQ$V5)]
TSS.Abias.A =  TSS.all[Abias.A]
TSS.Abias.D =  TSS.all[Abias.D]
TSS.Dbias.A =  TSS.all[Dbias.A]
TSS.Dbias.D =  TSS.all[Dbias.D]
...
pdf("plotDNS.AD1.AvsD.pdf")
plotMetaPair(bw, windowA=TSS.A, windowD=TSS.D, main="A vs D")
plotMetaPair(bw, windowA=TSS.Aog, windowD=TSS.Dog, main="A vs D")
dev.off()
pdf("plotDNS.AD1.AvsD.2.pdf")
